...
const Sidebar = () => {
    ...
    const filteredUsers = input ? users.filter((user) => user.fullName
        .toLowerCase().includes(input.toLowerCase())) : users
    ...
    return (
        ...
        text-white ${selectedUser ? "max-md:hidden" : ''}`}>
            <div className='pb-5'>
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <div className=' relative py-2 group '>
                        ...
                        {/* // ye div sb menu item ehega like edit or update */}
                        <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md 
                 bg=[#282142] border border-gray-600 text-gray-100 hidden bg-[#282142]
]
                ...
                {/* // yha pr Search vali field banani h */}
            ...
            {/*  yha pe user  details h slidebar me */}
            ...
            <div className='flex flex-col'>
                {filteredUsers.map((user, index) => (
                    <div onClick={() => {
                        ...
                        key={index} className={` relative flex items-center gap-2 p-2 pl-4 rounded-full
                     cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id
                            && 'bg-[#282142]/60'}`}>
                        {/* for pfofile photo */}
                        <img src={user?.profilePic || assets.avatar_icon} alt=''
                            className='rounded-full w-[35px] aspect-[1/1]' />
                        {/*  yha nam online and msg ko  eksath group krenge */}
                        <div className='flex flex-col leading-5'>
                            <p>{user.fullName}</p>
                            {
                                onlineUsers.includes(user._id)
                                    ? <span className='text-[#70BA4C] text-xs'>Online</span>
                                    : <span className='text-[#7B8089] text-xs'>Offline</span>
                            }
                        </div>
                        {
