...
function ChatContainer() {
    ...
    return selectedUser ? (
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            {/* --------------- chat area----------- */}
            <div className=' flex flex-col h-[calc(100%-125px)] overflow-y-scroll p-3
            pb-6'>
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId
                        ...
                        {msg.image ? (
                            ...
                            rounded-lg mb-8 break-all b-violet-500/30 text-white ${msg.senderId === authUser._id ?
                                    'rounded-br-none' : 'rounded-bl-none'}`}>
                                {msg.text}</p>
                        )}
                        {/* ----  ye dusre sender k liye h jese ki m  */}
                        <div className='text-center text-xs'>
                            <img src={msg.senderId === authUser._id ? authUser?.profilePic ||
                                assets.avatar_icon : selectedUser?.profilePic ||
                            assets.avatar_icon}
                                alt='' className='rounded-full w-7' />
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full '>
                    ...
                    <input onChange={handleSendImage} type='file' id='image' accept='image/png,image/jpeg,application/pdf' hidden />
                    <label htmlFor='image'>
                        <img src={assets.gallery_icon} alt='' className='w-5 mr-2 cursor-pointer' />

                    </label>
                </div>
                {/* ---------- ye send vale button k liye h ------- */}
        ...
        bg-white/10 max-md:hidden'>
            <img src={assets.chat_icon} className='max-w-40' alt='' />
